...
2026-09-08 数出来的：仓库里有 5 处手抄的原子写，**一处都没 fsync**。
replace 本身是原子的，但没 fsync 的话数据可能还在页缓存里没落盘；这台机器每天
被硬断电两次（早班断电重开 + 队列跑完自己关机），断在那个窗口，磁盘上留下的就是
一个内容为空的新文件——而它们写的都是 AUTO-MAS / OK-WW / MaaEnd 没有就起不来的配置。
