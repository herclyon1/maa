...
def run_case(label, *, file_ver, prev_log, kept, logs):
    ...
    if kept:
        state.mkdir(); (state / preupdate._VERSION_FILE).write_text(kept, encoding="utf-8")
    seq = list(logs)
...
check("确认后记下新版本", (state / preupdate._VERSION_FILE).read_text(encoding="utf-8"), NEW)
...
check("无更新不发通知", got, "")
check("但版本记下了", (state / preupdate._VERSION_FILE).read_text(encoding="utf-8"), NEW)
