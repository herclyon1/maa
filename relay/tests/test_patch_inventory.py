...
print("[在贴的清单是固定的九条，顺序也固定]")
check("在贴的", P.active_patches(), [
    "巢穴任务（整份文件替换）", P._STAMINA.name, P._NOFARM.name, P._CLAIM.name,
    P._TACETSHOT.name, P._NOWAVE.name, P._RETRYCAP.name, P._LETPASS.name, P._COUNT.name])
...
check("文档说清了在贴的条数", "9" in doc.split("Deliberately reverted")[0], True)
