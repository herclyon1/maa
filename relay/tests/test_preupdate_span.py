...
check("正常", preupdate_maaend._span("v1", "v2"), "v1 → v2")
check("同版=旧版没读到（不许吞成一个版本号）", preupdate_maaend._span("v2", "v2"), "（旧版本没读到）→ v2")
check("取旧版：跳过等于新版的", preupdate_maaend._pick_old(("", "v2", "v1", "v0"), "v2"), "v1")
check("取旧版：全等于新版→空", preupdate_maaend._pick_old(("v2", "", "v2"), "v2"), "")
check("旧版没读到要明说", preupdate_maaend._span("", "v2"), "（旧版本没读到）→ v2")
check("旧名字仍可用", preupdate_maaend._maaend_span("v1", "v2"), "v1 → v2")
...
check("读到", preupdate_maaend._maaend_file_version(TMP), "v2.26.0-beta.9")
check("没文件→空串", preupdate_maaend._maaend_file_version(TMP / "nope"), "")
(TMP / "bad").mkdir(); (TMP / "bad" / "interface.json").write_text("{", encoding="utf-8")
check("坏 JSON→空串", preupdate_maaend._maaend_file_version(TMP / "bad"), "")

print("[MAA：待装包名里的目标版本]")
maa = TMP / "maa"; maa.mkdir()
check("没包→空串", preupdate_maa._maa_pending_version(maa), "")
(maa / "MirrorChyanAppv6.17.0-beta.8.zip").write_bytes(b"")
check("从包名读", preupdate_maa._maa_pending_version(maa), "v6.17.0-beta.8")
...
for msg in [f"MAA 已更新：{preupdate_maaend._span('v6.17.0-beta.7', 'v6.17.0-beta.8')}",
            ...
            f"AUTO-MAS 有更新：{preupdate_maaend._span('v5.5.0-beta.1', 'v5.5.0')}（安装中，装完自动重启）",
            f"OK-WW 已更新：{preupdate_maaend._span('v3.6.6-beta.1', 'v3.6.6')}"]:
