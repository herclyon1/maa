...
(function () {
  ...
  const UNBUILT = { "BlurDistance*/BlurOpacity*": "the rim's blur reduction (three-stage r(d)): 待做 with the level-mix construction of topbar.js R3″ (not in R63)", "FaceColorMatrixMaxLuma (EDR)": "SDR screen: MaxLumaSDR used (k_EDR 0, §7.1)",
    "KeyFillHighlight* (in-shader)": "待读: Height .5333 + EffectOffset −.5333 → stroke_mode 1 (keyfill §5.1: |height + offset| < .001) — the stroke-mode branch of the shader is unread; the documented band is stroke_mode 0's", "ShadowColorMatrixWhite/Black/Saturation": "待读: how the four shadow colour keys enter set_ycc_composite (§7.3)", "Bleed capture edge": "the capture's marginWidth for the menu is unread — the alert's CaptureMargin 60.2 (→ 60) is taken: the capture box (panel ± 60, clamped to the copy) tiled and blurred σ 100 = its mean (R57′)" };
  Object.assign(BUILT, { "InnerRefraction*/OuterRefraction*/RefractionOpacity/RefractionDistance*": "R63: two feDisplacementMaps (maps from the supercircle SDF, formula §3 uv1/uv2) mixed by .6·sat((d + 1)/1)", "Bleed*": "R63 / R57′: the capture box's mean (feTile + σ 100 blur; lod 5.87 ≈ a 128-pt texel) displaced outward 58.45·Dc through the bleed YCC matrix, weight Opacity·w(d)·(luma or 1 − luma)⁴ as one 65-sample LUT (alert-native-formula §4 ⑦)", "highlight layer (menu-glass-sdfdump §3)": "R63: the KeyFill bands (main + diffuse, spread 1.5253) through the dumped vibrantColorMatrix, two stages" });
  ...
  const ensureFilter = (theme, W, H) => { const k = glassKeys(theme); let svg = document.getElementById("menu-glass-svg"); if (!svg) { svg = document.createElementNS(NS, "svg"); svg.id = "menu-glass-svg"; svg.setAttribute("width", "0"); svg.setAttribute("height", "0"); svg.style.cssText = "position:absolute;width:0;height:0"; document.body.appendChild(svg); }
    ...
    const CAPM = 60, M2 = 300, bleedBlur = Math.min(bleedSig, M2 / 3);
    const bleed = img(im.bleed, "mapb0") + `<feFlood flood-color="rgb(128,128,0)" result="midb"/><feComposite in="mapb0" in2="midb" operator="over" result="mapb"/>` + selCh("mapb", 2, "wb")
      + `<feOffset in="SourceGraphic" dx="0" dy="0" x="${-CAPM}" y="${-CAPM}" width="${im.W + 2 * CAPM}" height="${im.H + 2 * CAPM}" result="cap" data-menu-cap="1"/><feTile in="cap" result="tiled"/><feGaussianBlur in="tiled" stdDeviation="${bleedBlur.toFixed(2)}" result="bb"/>`
    ...
    svg.innerHTML = head("menu-glass-f1", 220) + `<feGaussianBlur in="SourceGraphic" stdDeviation="${k.BlurRadius * 4}" result="blur0"/>${refraction}</filter>`   /* f1's region 220 ≥ f2's 300 − its blurs' reach: f2 samples an opaque raster wherever its σ 100 tile blur reads */
      + head("menu-glass-f2", M2) + `${f2chain}${maxLuma}${faceCM}${bleed}</filter>`   /* the bleed's capture sample = this filter's SourceGraphic (the refracted σ20 blur, negligible under σ 293), its weight from `out` (R57′: it had been sampling the face output) */
  ...
  const placeGlassRest = (g, to) => { if (!g) return; const px = to.left - g.mr.left, py = to.top - g.mr.top, w = to.width, h = to.height;
    for (const id of ["menu-glass-f", "menu-glass-f0", "menu-glass-f1", "menu-glass-f2", "menu-glass-f3"]) { const fx = document.getElementById(id); if (!fx) continue; const m = +fx.dataset.margin || 120;   // each filter's own margin (f1 220, f2 300 for the bleed's tile blur, the rest 120 — the bleed map reaches ≤ 58.45 pt outside)
      ...
      const cap = fx.querySelector("[data-menu-cap]"); if (cap) { const cw = g.copy.offsetWidth, ch = g.copy.offsetHeight, x0 = Math.max(0, px - 60), y0 = Math.max(0, py - 60), x1 = Math.min(cw, px + w + 60), y1 = Math.min(ch, py + h + 60); cap.setAttribute("x", String(x0)); cap.setAttribute("y", String(y0)); cap.setAttribute("width", String(Math.max(1, x1 - x0))); cap.setAttribute("height", String(Math.max(1, y1 - y0))); } } };
