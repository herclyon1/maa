/* lens-engine-fix.js — 引擎校正 (engine correction) for the lens displacement maps, default ON (监督局 2026-09-19 08:4x).
   ...
   Apply after the <svg> with the filters is in the DOM; index.html and lens-test.html load it. Switch: <script data-engine-fix="off">
...
(function () {
  ...
  const k = 1 / (window.devicePixelRatio || 1);
  window.LENS_ENGINE_FIX_K = k;
  const fix = async (fe) => {
    ...
    const im = ctx.getImageData(0, 0, cv.width, cv.height), d = im.data, step = k * 255 / S;
