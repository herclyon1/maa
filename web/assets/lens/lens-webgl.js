...
void main(){
  ...
  const create = (canvasOrOpts, opts0) => {
    ...
    const setState = (s) => {
      ...
      stats.gpuMs = performance.now() - t0; stats.frames++; stats.set = wsel; if (!s._prewarm) { drawn = true; stats.last = { lift: p, pd, platterAlpha: (pl_.rgba[3] == null ? 1 : pl_.rgba[3]) * (pl_.alpha == null ? 1 : pl_.alpha), t: performance.now() }; }   /* stats.last: what the frame drew (R7's row reads the platter's alpha against 1 − lift) */
    };
