...
ACCEPT.add(async function sw({ check, num, col, sleep }) {
  ...
  if (window.Switch && Switch.flexOf) {
    ...
    const scOK = withF.every((s) => Math.abs(s.sc[0] - (1 + s.f.lift * (s.f.liftSX - 1)) * s.f.out.sx) < .01 && Math.abs(s.sc[1] - (1 + s.f.lift * (s.f.liftSY - 1)) * s.f.out.sy) < .01 && Math.abs(s.kdx - s.f.out.sx * s.f.out.dx) < .01);   // the driver's own UNCLAMPED lift (ζ .625 overshoots past 1 while the drag begins; the --lift var is clamped)
