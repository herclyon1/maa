...
BleedEdge: "近似: the capture box (panel ± 60.2, clamped to the copy) is TILED and blurred σ 100 for its mean — the native sampler's clamp_to_edge replicates the edge column instead; equal on a flat page", BlurFillTexelPair: "近似: the ±.75 mip-3 texel pair is taken on both axes (uv ± off with off = lod·(.25/size) read as a float2, menu-card-material §7c); one-axis or the base-texel reading (±3 pt) would differ only on structured content" };
  ...
  const ensureLight = (W, H, th) => { const k = keysFor(th); let svg = document.getElementById("alert-glass-svg0"); if (!svg) { svg = document.createElementNS(NS, "svg"); svg.id = "alert-glass-svg0"; svg.setAttribute("width", "0"); svg.setAttribute("height", "0"); svg.style.cssText = "position:absolute;width:0;height:0"; document.body.appendChild(svg); }
    const sig = W + "x" + H + "/" + th; if (svg.dataset.size === sig) return; svg.dataset.size = sig;
    ...
    svg.innerHTML = `<filter id="alert-glass-f0" filterUnits="userSpaceOnUse" x="${-M}" y="${-M}" width="${W + 2 * M}" height="${H + 2 * M}" color-interpolation-filters="sRGB" data-theme="${th}" data-margin="${M}" data-sigma="${sig2.toFixed(3)}">`
      ...
      + maxLumaChain("cbf", comp, luma) + `<feColorMatrix in="ml" type="matrix" values="${faceMatrix(k)}" result="face"/></filter>`; };
  ...
  const glass = { layer: null, copy: null, w2: null, w3: null, theme: null, stroke: null, ring: null, light: false, warmedAt: 0 };
  ...
  const build = () => { const th = theme(); const main = document.getElementById("app"), pane = dlg.querySelector(".pane"); if (!main || !pane) return; strip(); const W = dlg.offsetWidth, H = dlg.offsetHeight, warm = !!imageCache[W + "x" + H];
    ...
    const layer = document.createElement("div"); layer.className = "alert-glass"; const copy = main.cloneNode(true); copy.removeAttribute("id"); copy.querySelectorAll("[id]").forEach((e) => e.removeAttribute("id")); copy.querySelectorAll("canvas, .lens-clip, script, .menu, .menu-scrim, dialog").forEach((e) => e.remove());
    copy.className = "alert-glass-copy"; copy.setAttribute("aria-hidden", "true"); copy.inert = true; const mr = main.getBoundingClientRect(); copy.style.cssText = `position:absolute;left:0;top:0;width:${mr.width}px;min-height:${Math.max(mr.height, innerHeight + 200)}px;pointer-events:none;background:${getComputedStyle(document.body).backgroundColor};filter:url(#alert-glass-${warm ? "f1" : "f0"})`;   // the page colour under #app (body's background; #app paints none): without it the copy's gaps are transparent and the live page shows through the glass
  ...
  const strip = () => { if (glass.layer) glass.layer.remove(); if (glass.stroke) glass.stroke.remove(); glass.layer = glass.copy = glass.w2 = glass.w3 = glass.stroke = glass.ring = null; glass.light = false; };
