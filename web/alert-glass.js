...
onmessage = async (e) => { const { key, gen, W, H, KEYS, HL } = e.data; try { const t0 = performance.now(), px = mapPixels(W, H, KEYS, HL), t1 = performance.now(); let urls = null;
  ...
  const strokeFilter = (th, W, H, r) => { const k = keysFor(th), dpr = Math.min(3, Math.max(1, window.devicePixelRatio || 1)), m = strokeMap(W, H, r, k, dpr), E = m.E, comp = 1 - k.FaceColorMatrixMaxLumaSDR, luma = ".2126 .7152 .0722", bias = -k.KeyFillHighlightColorBias, qmax = 9 / 8;
    ...
    svg.innerHTML = `<filter id="alert-stroke-f" data-rings="${rings.length}" filterUnits="userSpaceOnUse" x="0" y="0" width="${W + 2 * E}" height="${H + 2 * E}" color-interpolation-filters="sRGB" data-theme="${th}" data-e="${E}" data-dpr="${dpr}" data-kside="${m.kside.toFixed(3)}" data-ktop="${m.ktop.toFixed(3)}" data-s="${Math.cos(k.KeyFillHighlightSpreadSDR).toFixed(4)}" data-bias="${bias}" data-dimming="${k.Dimming}">`
      + `<feImage href="${m.href}" preserveAspectRatio="none" x="0" y="0" width="${W + 2 * E}" height="${H + 2 * E}" result="k0" data-stroke-img="1"/>` + rings.map((_, i) => ring(i, `<feFlood flood-color="rgb(0,0,0)" flood-opacity="${k.Dimming}" result="dim"/><feComposite in="dim" in2="SourceGraphic" operator="over" result="src"/>`
  ...
  const buildStroke = () => { if (!glass.copy || !dlg.open || glass.stroke) return; const t0 = performance.now(), main = document.getElementById("app"); if (!main) return; const th = glass.theme, k = keysFor(th), W = dlg.offsetWidth, H = dlg.offsetHeight, r = k.CornerRadius, f = strokeFilter(th, W, H, r), E = f.E, fw = 1 / f.dpr, h = k.KeyFillHighlightHeight;
    ...
    copy.style.cssText = `position:absolute;left:${mr.left - (pl - E)}px;top:${mr.top - (pt - E)}px;width:${mr.width}px;min-height:${Math.max(mr.height, innerHeight + 200)}px;pointer-events:none;background:${getComputedStyle(document.body).backgroundColor}`;
    /* 数据 09-25 00:34: the filter sits on a wrapper at the layer's origin, its region / image / ring strips from 0 — Safari moved the filter's result right / down
       by a positive region origin (on the copy: x = px − E = 57, y = py − E = 161.8, only the bottom-right of the ring showed; flood test, simulator A) */
    const wrap = document.createElement("div"); wrap.style.cssText = `position:absolute;left:0;top:0;width:${W + 2 * E}px;height:${H + 2 * E}px;pointer-events:none;filter:url(#alert-stroke-f)`;
    wrap.appendChild(copy); el.appendChild(wrap); if (!el.parentNode) dlg.insertBefore(el, dlg.firstChild); glass.stroke = el; glass.strokeMs = performance.now() - t0; };   // strokeMs: the .settled handler's own main-thread time (accept-alert)
  ...
  const build = () => { const th = theme(); const main = document.getElementById("app"), pane = dlg.querySelector(".pane"); if (!main || !pane) return; strip(); const W = dlg.offsetWidth, H = dlg.offsetHeight, warm = !!imageCache[W + "x" + H];
    ...
    refilter(copy);
    if (!warm) { glass.light = true; imagesAsync(W, H).then(() => { if (glass.copy !== copy || !dlg.open) return; ensureFilter(W, H, th); copy.style.filter = "url(#alert-glass-f1)"; w2.style.filter = "url(#alert-glass-f2)"; w3.style.filter = "url(#alert-glass-f3)"; glass.light = false; glass.warmedAt = performance.now(); place(); refilter(copy); }); } };   // R57‴: the full chain once the maps are made — in the map worker (界面 09-23), the light chain meanwhile
