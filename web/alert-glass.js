...
onmessage = async (e) => { const { key, gen, W, H, KEYS, HL } = e.data; try { const t0 = performance.now(), px = mapPixels(W, H, KEYS, HL), t1 = performance.now(); let urls = null;
  ...
  const glass = { layer: null, copy: null, page: null, w2: null, w3: null, theme: null, stroke: null, strokeHost: null, ring: null, light: false, warmedAt: 0 };
  const place = () => { if (!glass.copy || !dlg.open) return; const dr = dlg.getBoundingClientRect(), mr = document.getElementById("app").getBoundingClientRect(); const M = 120, W = dlg.offsetWidth, H = dlg.offsetHeight;
    ...
    for (const id of ["alert-glass-f0", "alert-glass-f1", "alert-glass-f2", "alert-glass-f3"]) { const f = document.getElementById(id); if (!f) continue; const m = +f.dataset.margin || M, g = +f.dataset.gres || 1; f.setAttribute("x", String((px - m) / g)); f.setAttribute("y", String((py - m) / g)); for (const im of f.querySelectorAll("feImage")) { im.setAttribute("x", String(px / g)); im.setAttribute("y", String(py / g)); }
  ...
  const strip = () => { if (glass.layer) glass.layer.remove(); if (glass.stroke) glass.stroke.remove(); if (glass.strokeHost) glass.strokeHost.remove(); glass.layer = glass.copy = glass.page = glass.w2 = glass.w3 = glass.stroke = glass.strokeHost = glass.ring = null; glass.light = false; };
  ...
  { const warmUp = () => { try { const sz = localStorage.getItem("ark-alert-size"); if (!sz) return; const [W, H] = sz.split("x").map(Number); if (W > 0 && H > 0 && !imageCache[sz]) imagesAsync(W, H).then(() => { glass.prewarmed = sz; });
        if (W > 0 && H > 0) idle(() => { warmStroke(W, H); glass.strokeWarmed = sz; }); } catch (e) {} };   // the stroke's k map too, in its own idle slot
