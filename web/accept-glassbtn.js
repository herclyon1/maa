...
(function () {
  ...
  ACCEPT.add(async function acceptGlassBtn(ctx) {
    ...
    const scaleOf = (el) => { const t = getComputedStyle(el).transform; const m = /matrix\(([^)]+)\)/.exec(t); return m ? parseFloat(m[1].split(",")[0]) : 1; };
    ...
    /* ② press */
    ev(btn, "pointerdown", cx, cy); await raf();
    const st = GlassBtn.state(btn); if (!st) { check("玻璃钮：按下后有状态", "有", "缺", false); return; }
    const hold = await sample(btn, 600, st.t0);
    num(`按下 scale 对 ζ.625/r.262 闭式 rms（${hold.length} 帧）`, 0, rms(hold.map((s) => s.x - closed(1, L, 0, 0.625, 0.262, s.t))), 0.005);
    num("按住 600 ms 后 scale = L", L, scaleOf(btn), 0.005);
    ...
    await sleep(600); check("松手后 transform 复位（ζ .375 settle ≈ 1.2 s）", "无", btn.style.transform || "无", !btn.style.transform);
    ...
    check("hidden 时按下态剥掉（transform 清）", "无", b2.style.transform || "无", !b2.style.transform);
