...
ACCEPT.add(async function nav({ check, num, sleep }) {
  ...
  const cssDone = (el, prop, cap = 600) => Promise.race([new Promise((r) => el.addEventListener("transitionend", (e) => { if (!prop || e.propertyName === prop) r(); }, { once: true })), (async () => { const t0 = performance.now(); while (performance.now() - t0 < cap) await tick(); })()]);
...
ACCEPT.add(async function navLargeTitle({ check, num, sleep }) {
  ...
  await settled(); await cssDone(h1, "opacity");   // topbar.css's h1 opacity transition (.2 s linear, --tb-large) runs once the drive's own value is removed
  ...
  await settled(); await cssDone(h1, "opacity");
