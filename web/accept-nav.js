...
ACCEPT.add(async function navLargeTitle({ check, num, sleep }) {
  ...
  { const s = tr[tr.length - 1], m = mat(h1), op = parseFloat(getComputedStyle(h1).opacity), cb = getComputedStyle(pback, "::before"), body = parseFloat(getComputedStyle(pback).opacity);
    ...
    /* G18 (数据 20:0x, uiprobe-motion-g18-C2..5.json, nav-native-formula.md 末「数据核 G18 / G21」): in iOS 27 the large title is not scaled or shifted during the
       push — the computed transform mid-push must be the identity (the title rides the header's parallax); the old R47′ scale made this row red */
    const idM = !m || (near(m[0], 1, 1e-4) && near(m[3], 1, 1e-4) && near(m[1], 0, 1e-6) && near(m[2], 0, 1e-6) && near(m[4], 0, .05) && near(m[5], 0, .05));
    check(`推入 +${s ? Math.round(s.el * 1000) : "?"} ms（p ${s ? s.p.toFixed(3) : "?"}）：大标题 transform 恒等（G18：iOS 27 大标题不缩放不移位，随页头视差走）`, "identity（p 在 0…1 之间）", m ? `matrix(${m.map((v, i) => i < 4 ? v.toFixed(3) : v.toFixed(1)).join(", ")})` : "none", !!s && s.p > .05 && s.p < .95 && idM);
