...
ACCEPT.add(async function navLargeTitle({ check, num, sleep }) {
  ...
  { const push = tr.filter((s) => s.dir > 0 && s.h1), K = N.KF, C = N.CHEV0;
    ...
    check(`R47′ 推入全程 ${push.length} 帧：关键帧常量 [0,.6] / [.5,1] / [.9,1]、.7×、(+7,+5) 逐帧成立；p ∈ (.9, .999) 有 ${late.length} 帧（弹出段真走过）；KF ${JSON.stringify(K)} CHEV0 ${JSON.stringify(C)}`, "on formula · late ≥ 3", `${onF ? "on formula" : "OFF"} · late ${late.length}`, push.length > 10 && onF && late.length >= 3);
  ...
  { const tr2 = N.state.trace, s = tr2[tr2.length - 1], m = mat(h1), op = parseFloat(getComputedStyle(h1).opacity), body = parseFloat(getComputedStyle(pback).opacity), cb = getComputedStyle(pback, "::before");
    check(`返回 +${s ? Math.round(s.el * 1000) : "?"} ms（p ${s ? s.p.toFixed(3) : "?"}）：大标题按同一 p 函数倒放（mode 5 镜像；Pop 关键帧时刻未读 待读）：scale / 平移 / alpha 对驱动器本帧`, s && s.h1 ? `${s.h1.sx.toFixed(3)} ${s.h1.sy.toFixed(3)} ${s.h1.dx.toFixed(1)} ${s.h1.dy.toFixed(1)} · α ${s.h1.a.toFixed(3)}` : "-", m ? `${m[0].toFixed(3)} ${m[3].toFixed(3)} ${m[4].toFixed(1)} ${m[5].toFixed(1)} · α ${op.toFixed(3)}` : "none", !!s && s.dir < 0 && !!s.h1 && !!m && near(m[0], s.h1.sx, .002) && near(m[3], s.h1.sy, .002) && near(m[4], s.h1.dx, .5) && near(op, s.h1.a, .01));
    check(`返回 +${s ? Math.round(s.el * 1000) : "?"} ms：返回钮本体随栏项淡出（1 − f_out(u₁)），chevron 内容 alpha 1、scale 1（Pop 的 chevron 关键帧未读）`, s ? `${s.back.a.toFixed(3)} · 1 · 1` : "-", `${body.toFixed(3)} · ${cb.opacity} · ${cb.scale}`, !!s && near(body, s.back.a, .01) && parseFloat(cb.opacity) === 1 && (cb.scale === "none" || parseFloat(cb.scale) === 1)); }
