...
ACCEPT.add(async function navLargeTitle({ check, num, sleep }) {
  ...
  { const tr2 = N.state.trace, pop = tr2.filter((s) => s.dir < 0 && s.h1), onP = pop.every((s) => { const u = 1 - s.p; return near(s.h1.a, seg(.5, 1, u), 1e-6) && near(s.back.ca, (1 - seg(0, .6, u)) * seg(.9, 1, s.p), 1e-6) && near(s.back.cs, .7 + .3 * seg(.9, 1, s.p), 1e-6) && near(s.back.kx, 7 * (1 - seg(.9, 1, s.p)), 1e-6); });
    ...
    check(`R69′ 返回全程 ${pop.length} 帧：大标题 α = seg[.5,1](u)、返回钮内容 α = (1 − seg[0,.6](u)) × chevron 段、chevron scale / 偏移按 p 的 [.9,1] 段逐帧成立；u ∈ (0, .2) 有 ${early.length} 帧（缩出段真走过；首帧 > 40 ms 只记不判）`, "on formula · early ≥ 1", `${onP ? "on formula" : "OFF"} · early ${early.length} · 首帧 ${Math.round(first * 1000)} ms${gap ? "（帧隔，不判 early）" : ""}`, pop.length > 10 && onP && (gap || early.length >= 1)); }
