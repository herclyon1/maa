    ...
    async function interactions() {
      ...
      if (q()) {
        ...
        { const a = cs(lens0, "::after"), sh = cs(lens0).boxShadow;
          ...
          check("分段抬起满：透镜内亮度 = 底（seg-lens-refraction §3 平灰 128 → 126：可见层不套 vibrantColorMatrix、不模糊）", "no filter", (cs(lens0, "::after").backdropFilter || "none") + " · " + cs(seg.querySelector(".warp")).filter.slice(0, 22), (cs(lens0, "::after").backdropFilter || "none") === "none" && /url/.test(cs(seg.querySelector(".warp")).filter));
        ...
        { const a0 = at(sel), a1 = at(unsel), lensD = seg.querySelector(".lens"); let wMax = 0, hMin = 99;
          ...
          await sleep(500); { const lr2 = lensD.getBoundingClientRect(); check("分段 G22 按住滑到另一段 +900 ms：透镜落定在手指下（中心 ± 3 pt）、220×44", `centre ${(ur.left + ur.width / 2).toFixed(0)} · ${((seg.getBoundingClientRect().width - 4) / bs().length + 24).toFixed(0)}×44`, `centre ${(lr2.left + lr2.width / 2).toFixed(1)} · ${lr2.width.toFixed(1)}×${lr2.height.toFixed(1)}`, Math.abs(lr2.left + lr2.width / 2 - (ur.left + ur.width / 2)) < 3 && Math.abs(lr2.height - 44) < 1 && Math.abs(lr2.width - ((seg.getBoundingClientRect().width - 4) / bs().length + 24)) < 1.5); } }
        ...
        await sleep(200); { const lr = seg.querySelector(".lens").getBoundingClientRect(), sr = seg.getBoundingClientRect(); check("分段松手 +320 ms：尺寸回到静止 198×28（ζ1/.25：+289 ms 达 99.4 %）", `${((sr.width - 4) / bs().length).toFixed(1)}×28`, `${lr.width.toFixed(1)}×${lr.height.toFixed(1)}`, Math.abs(lr.height - 28) < 0.6 && Math.abs(lr.width - (sr.width - 4) / bs().length) < 0.6); }
