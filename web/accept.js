    ...
    async function interactions() {
      ...
      if (q()) {
        ...
        { const lp = parseFloat(seg.style.getPropertyValue("--lp")), warp = seg.querySelector(".warp"), warpl = seg.querySelector(".warpl"), cb = warpl ? warpl.querySelectorAll(".copy .cb").length : 0;
          ...
          { const punch = warp && warp.querySelector(".disp .punch"), pcp = punch && punch.querySelector(".copy .cb"), SSa = window.LENS_SS || 1;
            check("分段抬起：底图复本 .warp 过 #seg-lens-f-bg-220、标签复本 .warpl 过 #seg-lens-f-lab-220（220 组；引擎校正 2 ← ui2 README §0.4 recipe：滤镜层 .dispf/.displf = 透镜盒 × SS overflow hidden，包裹 .disp/.displ 缩 1/SS，屏上宽 = 透镜盒）；B4-c' §4b：标签复本 = 整盒先裁胶囊（半径 × SS）再位移、无 196×28 门户；底图复本带 .punch（标签复本挖掉胶囊 = DestOut，clip-path evenodd）", `${bs().length} cb · dispf url(#seg-lens-f-bg-220) · displf url(#seg-lens-f-lab-220) · SS ${SSa} · no .portal · punch evenodd`, `${cb} cb · ${warp ? cs(warp.querySelector(".dispf")).filter.slice(0, 26) : "-"} · ${warpl ? cs(warpl.querySelector(".displf")).filter.slice(0, 27) : "-"} · ${warpl && warpl.querySelector(".portal") ? "portal!" : "no .portal"} · ${punch ? (cs(punch).clipPath.startsWith("path(evenodd") ? "punch evenodd" : cs(punch).clipPath.slice(0, 20)) : "no punch"} ${pcp ? "+labels" : ""}`, !!warp && !!warpl && cb === bs().length && bs().length === 2 && /inset/.test(cs(warp).clipPath) && /inset/.test(cs(warpl).clipPath) && /url\("?#seg-lens-f-bg-220"?\)/.test(cs(warp.querySelector(".dispf")).filter) && cs(warp.querySelector(".dispf")).overflow === "hidden" && Math.abs(warp.querySelector(".dispf").getBoundingClientRect().width - warp.getBoundingClientRect().width) < 0.5 && Math.abs(warp.querySelector(".dispf").offsetWidth - SSa * warp.getBoundingClientRect().width) < 1 && /url\("?#seg-lens-f-lab-220"?\)/.test(cs(warpl.querySelector(".displf")).filter) && !warpl.querySelector(".portal") && Math.abs(px(cs(warpl.querySelector(".displf")).borderRadius) - SSa * warpl.getBoundingClientRect().height / 2) < 1.5 * SSa && !!punch && cs(punch).clipPath.startsWith("path(evenodd") && !!pcp); }
          { const cp = warp && warp.querySelector(".copy"), cpl = warpl && warpl.querySelector(".copy"), bgw = cp && cp.querySelector(".cbgwrap");
            ...
            const fd = document.querySelector("#seg-lens-f-bg-220 feDisplacementMap"); check("分段抬起：位移量 scale = S × SS × 进度（S = 该组 filter 的 data-s，220 组 40 ← ui2 c4efe4b；SS = 引擎校正 2 的超采样倍数 window.LENS_SS，贴图 1 pt = SS 层像素 ← ui2 README §0.4 recipe step 2；同一条曲线）", `≈ 40·${window.LENS_SS || 1}·lp`, `${fd ? fd.getAttribute("scale") : "-"} (data-s ${document.querySelector("#seg-lens-f-bg-220").getAttribute("data-s")}, ss ${fd && fd.dataset.ss || 1})`, !!fd && document.querySelector("#seg-lens-f-bg-220").getAttribute("data-s") === "40" && Math.abs(parseFloat(fd.getAttribute("scale")) - 40 * (window.LENS_SS || 1) * lp) < 0.8 * (window.LENS_SS || 1));
        ...
        { check("分段抬起满：透镜内亮度 = 底（seg-lens-refraction §3 平灰 128 → 126：可见层不套 vibrantColorMatrix、不模糊）", "no filter", (cs(lens0).backdropFilter || "none") + " · " + cs(seg.querySelector(".warp .dispf")).filter.slice(0, 26), (cs(lens0).backdropFilter || "none") === "none" && /url/.test(cs(seg.querySelector(".warp .dispf")).filter));
