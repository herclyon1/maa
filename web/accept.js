    ...
    async function interactions() {
      ...
      if (q()) {
        ...
        { svgcheck("分段抬起满：透镜内亮度 = 底（seg-lens-refraction §3 平灰 128 → 126：可见层不套 vibrantColorMatrix、不模糊）", "no filter", (cs(lens0).backdropFilter || "none") + " · " + cs(seg.querySelector(".warp .disp")).filter.slice(0, 26), (cs(lens0).backdropFilter || "none") === "none" && /url/.test(cs(seg.querySelector(".warp .disp")).filter));
          ...
          if (GLON) { const c = seg.querySelector("canvas.glens"), g = seg.__gl, sr = seg.getBoundingClientRect(), cr = c && c.getBoundingClientRect(), lr = lens0.getBoundingClientRect(), fr0 = g ? g.lens.stats.frames : 0;
            ...
            check("分段 WebGL：白平台淡出 = .lens 自身在 canvas 之上（z 10，opacity = 1 − lp，_controlForegroundColor）——shader 暂无平台层（§4b 层序里它在位移底图之上、边线之下，待 2号 uniform）", "z 10 · opacity 1 − lp", `z ${cs(lens0).zIndex} · ${cs(lens0).opacity} vs ${(1 - lpNow).toFixed(3)}`, cs(lens0).zIndex === "10" && Math.abs(parseFloat(cs(lens0).opacity) - (1 - lpNow)) < 0.02);
            ...
            const F1 = window.LensWebGL ? LensWebGL.FS1 + LensWebGL.FS2 + LensWebGL.COMMON : "";
