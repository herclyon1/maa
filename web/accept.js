      ...
      if (q()) {
        ...
        { const lp = parseFloat(seg.style.getPropertyValue("--lp")), warp = seg.querySelector(".warp"), warpl = seg.querySelector(".warpl"), cb = warpl ? warpl.querySelectorAll(".copy .cb").length : 0;
          ...
          { const cp = warp && warp.querySelector(".copy"), cpl = warpl && warpl.querySelector(".copy"), bgw = cp && cp.querySelector(".cbgwrap");
            ...
            const pl = seg.querySelector(".plat"); check("分段抬起：白平台层 .plat 在底复本之上、标签复本之下（z 5 / 6），opacity = 1 − lp（restingBackground 1 → 0，seg-keys -platter-keys）；底复本 opacity = DestOut（前 3 帧到 1，-destout-keys）；标签复本不淡", "1−lp · warp 1 · warpl 1 · z 5/6", `${pl ? cs(pl).opacity : "-"} · ${cs(warp).opacity} · ${cs(warpl).opacity} · z ${pl ? cs(pl).zIndex : "-"}/${cs(warpl).zIndex}`, !!pl && Math.abs(parseFloat(cs(pl).opacity) - (1 - lp)) < 0.03 && parseFloat(cs(warp).opacity) > 0.95 && cs(warpl).opacity === "1" && cs(pl).zIndex === "5" && cs(warpl).zIndex === "6"); }
        ...
        { const a = cs(lens0, "::after"), sh = cs(lens0).boxShadow;
          ...
          check("分段抬起满：透镜内亮度 = 底（seg-lens-refraction §3 平灰 128 → 126：可见层不套 vibrantColorMatrix、不模糊）", "no filter", (cs(lens0, "::after").backdropFilter || "none") + " · " + cs(seg.querySelector(".warp .disp")).filter.slice(0, 26), (cs(lens0, "::after").backdropFilter || "none") === "none" && /url/.test(cs(seg.querySelector(".warp .disp")).filter));
          ...
          { const capB = cs(rimEl, "::before"), capA = cs(rimEl, "::after"), csh = capB.boxShadow.replace(/\s+/g, " ");
            check("分段抬起满：.rim 层在最上（z 7）按 seg-lens-drag-mid.md §2 剖面画：外侧 1 pt 暗线 = 外环 .10 + 底 1 px 加深 .026；上下长边 1 pt 亮线 + 暗带 = 每 pt 一档的渐变（--seg-rim-top/-glow/-bot）；两端 = 半圆帽 ::before/::after（外环 .02、内环 .083、11 px 暗带 .06）；不再有固定 α 黑圈；opacity = --lp", "ring .10 + .026 · 3 gradients · caps · z 7", `${rsh.slice(0, 80)} · ${(cs(rimEl).backgroundImage.match(/linear-gradient/g) || []).length} gradients · caps ${capB.content !== "none" && capA.content !== "none" ? "2" : "-"} ${csh.slice(0, 40)} · z ${cs(rimEl).zIndex}`,
              !!rimEl && cs(rimEl).zIndex === "7" && /^rgba\(0, 0, 0, 0\.1\) 0px 0px 0px 1px, rgba\(0, 0, 0, 0\.02[67]\) 0px 1px 0px 0px$/.test(rsh) && (cs(rimEl).backgroundImage.match(/linear-gradient/g) || []).length === 3 && capB.content !== "none" && capA.content !== "none" && /inset/.test(csh) && /0px 0px 0px 1px inset/.test(csh) && Math.abs(parseFloat(cs(rimEl).opacity) - 1) < 0.03); } }
        ...
        { const a0 = at(sel), a1 = at(unsel), lensD = seg.querySelector(".lens"); let wMax = 0, hMin = 99;
          for (let k = 1; k <= 10; k++) { pev(seg, "pointermove", [a0[0] + (a1[0] - a0[0]) * k / 10, a1[1]]); await sleep(16); const r = lensD.getBoundingClientRect(); wMax = Math.max(wMax, r.width); hMin = Math.min(hMin, r.height); }   // the native abc recording: 10 × 20 pt every ~17 ms
          ...
          check("分段 G22 按住滑到另一段 +400 ms：透镜到手指下（跟踪弹簧过冲中，中心 ± 12 pt；原生 B 段过冲 10 pt）、index 不改", `${onText()} centre ${(ur.left + ur.width / 2).toFixed(0)}`, `${onText()} centre ${(lr.left + lr.width / 2).toFixed(1)} ${lr.width.toFixed(1)}×${lr.height.toFixed(1)}`, onText() === sel.textContent && Math.abs(lr.left + lr.width / 2 - (ur.left + ur.width / 2)) < 12 && renders === r1);
          await sleep(500); { const lr2 = lensD.getBoundingClientRect(); check("分段 G22 按住滑到另一段 +900 ms：透镜落定在手指下（中心 ± 3 pt）、220×44", `centre ${(ur.left + ur.width / 2).toFixed(0)} · ${((seg.getBoundingClientRect().width / bs().length - 4) + 24).toFixed(0)}×44`, `centre ${(lr2.left + lr2.width / 2).toFixed(1)} · ${lr2.width.toFixed(1)}×${lr2.height.toFixed(1)}`, Math.abs(lr2.left + lr2.width / 2 - (ur.left + ur.width / 2)) < 3 && Math.abs(lr2.height - 44) < 1 && Math.abs(lr2.width - ((seg.getBoundingClientRect().width / bs().length - 4) + 24)) < 1.5); } }
        ...
        await sleep(200); { const lr = seg.querySelector(".lens").getBoundingClientRect(), sr = seg.getBoundingClientRect(); check("分段松手 +320 ms：尺寸回到静止 196×28（ζ1/.25：+289 ms 达 99.4 %）", `${((sr.width / bs().length - 4)).toFixed(1)}×28`, `${lr.width.toFixed(1)}×${lr.height.toFixed(1)}`, Math.abs(lr.height - 28) < 0.6 && Math.abs(lr.width - (sr.width / bs().length - 4)) < 0.6); }
