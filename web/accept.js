    ...
    async function interactions() {
      ...
      if (q()) {
        ...
        { check("分段抬起满：透镜内亮度 = 底（seg-lens-refraction §3 平灰 128 → 126：可见层不套 vibrantColorMatrix、不模糊）", "no filter", (cs(lens0).backdropFilter || "none") + " · " + cs(seg.querySelector(".warp .disp")).filter.slice(0, 26), (cs(lens0).backdropFilter || "none") === "none" && /url/.test(cs(seg.querySelector(".warp .disp")).filter));
          const rb = seg.querySelector(".rimb"), hk = seg.querySelector(".hlk"), hw = seg.querySelector(".hlw"), lpNow = parseFloat(seg.style.getPropertyValue("--lp"));
          check("分段抬起满 B6：边线分两层——.rimb（标签复本之下 z 6：内阴影 + ring shadow + 外侧暗线）、.hlk/.hlw（之上 z 8：高光黑栈 / 白 plus-lighter 栈），随 --lp 淡入", "z 6 / 8 / 8 · plus-lighter · lp", `z ${rb ? cs(rb).zIndex : "-"} / ${hk ? cs(hk).zIndex : "-"} / ${hw ? cs(hw).zIndex : "-"} · ${hw ? cs(hw).mixBlendMode : "-"} · ${hw ? cs(hw).opacity : "-"}`, !!rb && !!hk && !!hw && cs(rb).zIndex === "6" && cs(hk).zIndex === "8" && cs(hw).zIndex === "8" && cs(hw).mixBlendMode === "plus-lighter" && cs(hk).mixBlendMode === "normal" && Math.abs(parseFloat(cs(hw).opacity) - lpNow) < 0.03 && Math.abs(parseFloat(cs(rb.firstElementChild).opacity) - lpNow) < 0.03);
          ...
          { const m = /^rgba\(0, 0, 0, 0\.043\) 0px ([\d.]+)px ([\d.]+)px 0px inset$/.exec(ishS); check("分段抬起满 B6 ④：内阴影 #21 = inset 0 8·lp 5.6·lp 黑 .043（§5.2 测量值：平台 .043、半值 top+8、σ 2.8 → blur 5.6）", `rgba(0,0,0,.043) 0 ${(8 * lpNow).toFixed(2)} ${(5.6 * lpNow).toFixed(2)} inset`, ishS, !!m && Math.abs(parseFloat(m[1]) - 8 * lpNow) < 0.1 && Math.abs(parseFloat(m[2]) - 5.6 * lpNow) < 0.1); }
          ...
          const hkR = hk ? [...hk.querySelectorAll("rect")] : [], hwR = hw ? [...hw.querySelectorAll("rect")] : [];
          check("分段抬起满 B6 ①：内侧亮线 + 高光尾 = #36 层公式：黑栈 stroke-opacity .0882·α / 白栈 .1471·α，13 环（主带 3 + 漫射 10），首环 α 1.678（.875 + .803），弧段用渐变描边", "13 + 13 · edge .1480 / .2469", `${hkR.length} + ${hwR.length} · edge ${hkR.length ? (parseFloat(hkR[0].getAttribute("stroke-opacity")) + parseFloat(hkR[1].getAttribute("stroke-opacity"))).toFixed(4) : "-"} / ${hwR.length ? (parseFloat(hwR[0].getAttribute("stroke-opacity")) + parseFloat(hwR[1].getAttribute("stroke-opacity"))).toFixed(4) : "-"} · ${hkR.length && /url/.test(hkR[0].getAttribute("stroke")) ? "gradient" : "-"}`,
            hkR.length === 13 && hwR.length === 13 && Math.abs(parseFloat(hkR[0].getAttribute("stroke-opacity")) + parseFloat(hkR[1].getAttribute("stroke-opacity")) - 0.0882 * 1.678) < 0.002 && Math.abs(parseFloat(hwR[0].getAttribute("stroke-opacity")) + parseFloat(hwR[1].getAttribute("stroke-opacity")) - 0.1471 * 1.678) < 0.002 && hkR.every((r) => /url/.test(r.getAttribute("stroke"))) && Math.abs(parseFloat(hkR[0].getAttribute("stroke-width")) - 1 / 3) < 0.001);
