    ...
    async function interactions() {
      ...
      if (q()) {
        ...
        { const L = window.__segLens, lp = parseFloat(q().style.getPropertyValue("--lp") || "0"), el = L && L.since_up != null ? L.since_up : (performance.now() - t0) / 1000, c = L ? L.x_screen : NaN, lw = L ? L.w_screen : NaN, lr = { width: lw };
          ...
          check("分段 G1 抬手 +272 ms：玻璃滑行中（--lp ≥ .8）、拉长（原生 seg-native-tap-frames.json 同时刻插值：中心 / 宽，控件坐标；容 ± 12；按驱动器本帧：__segLens.since_up / w_screen / x_screen，A15）", `lp ≥ .8 · w ${nw.toFixed(1)} ± 12 · centre ${nc.toFixed(1)} ± 12 @ ${el.toFixed(3)}`, `lp ${lp.toFixed(3)} · w ${isNaN(lw) ? "-" : lw.toFixed(1)} · centre ${isNaN(c) ? "-" : c.toFixed(1)} @ tick ${L ? L.tick : "-"}`, !!L && lp >= .8 && Math.abs(lw - nw) <= 12 && Math.abs(c - nc) <= 12); }
