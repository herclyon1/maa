...
and published with the page (`deploy-web.sh` copies `web/` recursively for it). It pulls
`card/detail` for the owned operators and equipped weapons, then `rules?charIds=<one>`
per operator and `rules?weaponIds=<one>` per weapon, and sums breakthroughs + skills to
12 + levels 1 -> 90 (the 完美 template of §7, talents excluded). A material whose count
is the same for every operator (or every weapon) is 通用 with that exact count; the rest
(high-tier materials, leaves, mushrooms, ores) carries the **largest** count any single
operator or weapon uses and a `note` listing the options, so 人份 = stock ÷ need reads
conservatively. The two exp rows (`exp:char`, `exp:weapon`) are virtual: the page folds
the exp materials into them by `count × exp`. Rerun the script when a new operator or
weapon appears; `--offline DIR` rebuilds from saved responses (the test's fixtures in
`relay/tests/fixtures/skland-endfield/`, `--subset --save-fixtures` writes them).
