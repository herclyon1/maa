...
| Reminders sidebar (Mac; identical tiles on iOS) | 2×2 summary tiles: white card, coloured circle with a white symbol top-left, big number top-right in the label colour, grey label below | the numbers on 状态: 今天跑了几趟 / 理智 / 波片 - a number that is only read is a Reminders tile; a coloured full-bleed tile is Shortcuts' 「tap to run」 and would promise a tap |
