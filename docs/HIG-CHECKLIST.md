...
| selection lens (tab bar `.glide`) | gaussianBlur inputRadius 2 + colorMatrix ≈ saturate 1.19 with additive −0.2 (dark 1.13 / −0.07) → `blur(2px) saturate(1.19)` + black overlay α .2 (.07) | `--ios-lens-blur/-saturate/-offset` (`--lens-filter/-fill`) | UIProbe deep `_UITabSelectionView` |
| segmented selected segment (`.segctl .lens`) | white platform (`--ios-card-bg`) with the old page's shadow, **等原值**: reference 34 shows a white segment, the tab-bar lens matrix made it dark grey (acceptance rejected); the data session is reading `--ios-segment-selected-*` by injection | `--ios-card-bg` | reference 34; shadow 无来源 until the original arrives |
