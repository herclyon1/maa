...
| selection lens (tab bar `.glide`, segmented `.lens`) | gaussianBlur inputRadius 2 + colorMatrix ≈ saturate 1.19 with additive −0.2 (dark 1.13 / −0.07) → `blur(2px) saturate(1.19)` + black overlay α .2 (.07) | `--ios-lens-blur/-saturate/-offset` (`--lens-filter/-fill`) | UIProbe deep `_UITabSelectionView`; the segmented lens dumps only destOut, so it shares the tab-bar recipe (acceptance's call) |
