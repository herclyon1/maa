...
| glass material (alert, glass buttons, tab-bar platter, menu, toast) | glassBackground inputBlurRadius 5 + vibrantColorMatrix ≈ saturate 1.2 with additive +0.147 → CSS `backdrop-filter: blur(5px) saturate(1.2)` + white overlay α .147; SDF highlight amount .5 → .5px white inset ring; diffuse height 8 / amount .15 → `0 8px 16px rgba(0,0,0,.15)` | `--ios-glass-blur/-saturate/-lift` `--ios-glass-highlight-amount` (page aliases `--glass-filter/-fill/-rim/-diffuse`) | UIProbe deep (UISDFBackdropView / UISDFView, remote-ref/deep-materials.md); Kit 89 「white 70 % + blur 30」 is superseded; menu and toast are not measured - the glass recipe is the nearest original |
...
| alert button fill | white background view + vibrantColorMatrix additive −0.12 (dark +0.11) → black overlay α .12 / white α .11 | `--ios-alert-button-offset` (`--alert-button-fill`) | UIProbe deep `_UIVisualEffectBackgroundView` |
