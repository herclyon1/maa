...
| selection lens (tab bar `.glide`) | gaussianBlur inputRadius 2 + the read 4×5 colorMatrix, decomposed exactly into brightness × saturate (light 1.13 × 1.062, dark 0.87 × 1.379) + additive offset −0.2 (dark −0.07) → `backdrop-filter: blur(2px) saturate() brightness()` and a `::after` overlay of rgb(1+e) with `mix-blend-mode: plus-darker` (exact on WebKit; Chrome falls back to a black overlay α \|e\|) | `--ios-lens-blur/-saturate/-brightness/-offset` (`--lens-filter`, `--lens-offset-color/-fallback`) | UIProbe deep `_UITabSelectionView`, matrix in `--ios-lens-matrix` |
