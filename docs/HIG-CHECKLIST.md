...
| Navigation bars | large title collapses into a compact bar on scroll | `.topbar` (glass, 44pt, 17 semibold centred) is driven by scroll position: its glass and hairline appear as content slides under it (`--bar`), the small title fades and rises in step with the large title crossing it (`--title`) - no toggle, no jump |
