...
| segmented control behaviour | like UISegmentedControl: the selection commits on release (pointerup / click) and the content switches synchronously (~2–8 ms, one render); the lens glides by CSS transition across the re-render; a press that slides across segments selects the segment under the finger on release; ten alternating taps 80 ms apart end on the last tap and never flip back | — | accept.js interaction items (2026-09-18: the 0.55 s setTimeout before `change` and the stale-select timers were the bug he caught live) |
