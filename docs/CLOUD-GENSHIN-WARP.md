...
源码里 `videoMode` 默认就是「画质优先」,`graphicsMode` 默认「高画质」,帧率默认 60。
不要再建议去悬浮球里改设置,那里没有能提升码率的开关。

服务器只拿到一个判断依据:测速上报的延迟。带宽测试被 `hideBandWidth: true` 关掉了,上报的带宽恒为 0。
而上报的延迟是虚高的:真实往返 267 毫秒,但 SDK 一次性发 20 个探测包,回包以约 15 毫秒的间隔挤出来,
平均值被算成 450~535 毫秒。把发送间隔拉开反而收不全回包,所以这个虚高改不掉。
