...
echo "▶ 2/15 不许用 powershell 5.1（要用 pwsh 7）"
...
echo "▶ 3/15 ssh 送 PowerShell 必须走 base64"
...
echo "▶ 9/15 静态检查（pyflakes + 3.14 严格编译）"
...
if hits=$(grep -rnE 'ark-relay[\\/]state[\\/][A-Za-z0-9_.-]+' scripts/win scripts/mac 2>/dev/null \
...
miss=0
for f in scripts/mac/*.sh scripts/win/*.py scripts/win/*.bat; do
  [ -e "$f" ] || continue
  base=$(basename "$f")
...
echo "▶ 15/15 文档自检（check-docs --local）"
