...
| **重算「一个干员满练要多少」表（库存页用）** | `scripts/mac/build-need-tables.py` —— 出新干员/新武器时跑一次，从森空岛 rules 逐人求和写 `web/data/need.json`，然后 `deploy-web.sh`。数字的坑（90 级 -1 哨兵、名字带换行、没有的材料不在 itemCount 里）见 `docs/SKLAND-API.md` §8 |
