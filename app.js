...
function numTiles(snap) {
  ...
  if (r) {
    /* The tab bar carries each game's own icon; the tile shows the resource - 理智 as a
       brain, 波片 as the crystal cube it is in the game - so the two never repeat. */
    h += numTile("brain.fill", "#0088ff", ak["理智"], ak["上限"] != null ? `/${ak["上限"]}` : "", "明日方舟 理智", whenFull(ak["回满"]) || (ak["理智"] >= ak["上限"] ? "已满" : ""), ak["错误"]);
    h += numTile("brain.fill", "#ff9500", ef["理智"], ef["上限"] != null ? `/${ef["上限"]}` : "", "终末地 理智", whenFull(ef["回满"]) || (ef["理智"] >= ef["上限"] ? "已满" : ""), ef["错误"]);
    h += numTile("cube.fill", "#30b0c7", ww["波片"], ww["上限"] != null ? `/${ww["上限"]}` : "", "鸣潮 波片",
